...
const editarTarefa = (estado: EstadoAplicacao, tarefa: Tarefa) => {
  return { ...estado, editando: !estado.editando, tarefaSelecionada: tarefa }
}

const atualizarTarefa = (estado: EstadoAplicacao, descricao: string): EstadoAplicacao => {
...
const atualizarUI = () => {
  ...
  const btnDeleteTask = document.querySelector<HTMLButtonElement>('.app__form-footer__button--delete');

  const btnDeletarConcluidas = document.querySelector('#btn-remover-concluidas') as HTMLButtonElement;
  const btnDeletarTodas = document.querySelector('#btn-remover-todas') as HTMLButtonElement;
